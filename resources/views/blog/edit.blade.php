...
<div class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-gray-900 sm:items-center py-4 sm:pt-0">
    @if (Route::has('login'))
        <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
            @auth
                <a href="{{ url('/') }}" class="text-sm text-gray-700 dark:text-gray-500 underline"><?=$title?></a>

                <a href="{{ url('/') }}" class="text-sm text-gray-700 dark:text-gray-500 underline">Home</a>
                <a href="{{ url('/auth/logout') }}" class="text-sm text-gray-700 dark:text-gray-500 underline">Logout</a>
    ...
    <div class="container">
        <div class="card">
            @if($create)
                <button onclick="addOpenModal()" class="btn btn-success">Add</button>
            @endif
                ...
                <form action="blogs/update" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="publish">
                            <option value="0" @if($data->publish == 0) selected @endif></option>
                            <option value="1" @if($data->publish == 1) selected @endif></option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="title">Status</label>
                        <select class="form-control" name="status">
                            <option value="0" @if($data->status == 0) selected @endif></option>
                            <option value="1" @if($data->status == 1) selected @endif></option>
                        </select>
                    </div>
                    <div class="form-group">
